...
export interface CustomersState extends EntityState<ProjectModel> {
  listLoading: boolean;
...
export function customersReducer(state = initialCustomersState, action: CustomerActions): CustomersState {
  switch (action.type) {
    ...
    case CustomerActionTypes.CustomersStatusUpdated: {
      ...
      const _partialCustomers: Update<ProjectModel>[] = [];
...
export const getCustomerState = createFeatureSelector<ProjectModel>('customers');
